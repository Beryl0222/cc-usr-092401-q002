...
- **误报申诉**：申诉打开即事件化冻结尚未执行的对外动作（`action_frozen`，显式 `frozen` 并保留原审批依据），敏感材料仅法务复核员与值班主管可见，阻断新的对外动作；申诉**驳回**只恢复当前仍具备授权的动作（冻结期间授权被撤回则保持冻结，重新授权齐备后自动恢复），申诉**成立**不可逆地**取消**全部待执行项（`action_cancelled`）并关闭案件；已执行动作只保留事实、绝不回滚。执行请求与申诉并发时由账本 `seq` 决定唯一先后：申诉先落账则执行必被 409 拦截，绝无外发；授权撤回、事件合并、进程重启均不能绕过冻结。迟到平台回执只能补充既有事实（回执/删除/改名），不复活已取消动作、不改变关闭结论。
